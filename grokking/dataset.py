...
class ModularArithmetic(Dataset):
    ...
    def __init__(self, data: Optional[torch.Tensor]=None, labels: Optional[torch.Tensor]=None, root: Optional[str]=None, metadata: Metadata={}) -> None:
        """
    ...
    def generate(
        ...
        metadata = cls.Metadata(operator=operator, modulus=modulus, frac_label_noise=frac_label_noise, seed=seed, shuffle=shuffle)
    ...
    def split(self, frac_train: float = 0.8):
        ...
        return (
            ModularArithmetic(self.data[:train_len], self.labels[:train_len], metadata=train_metadata), 
            ModularArithmetic(self.data[train_len:], self.labels[train_len:], metadata=val_metadata)
        )
    ...
    def generate_split(
        ...
        """
        if shuffle is False:
            warnings.warn("Shuffling is disabled. This will result in poorly separated training and validation sets.")
